...
public class T13_ThreadStopTest {
	/*
	 * Thread의 stop() 메서드를 호출하면 스레드가 바로 멈춘다. 이때 사용하던 자원을 정리하지 못하고 프로그램이 종료되어서 나중에
	 * 실행되는 프로그램에 영향을 줄 수 있다. 그래서 현재 stop()메서드는 비추천(Deprecated)으로 되어 있다.
	 */
...
//		 } catch(InterruptedException ex) {}
		...
		while (true) {
			...
			/*// 검사방법1 => 스레드의 인스턴스 객체용 매서드를 이용하는 방법
			if (this.isInterrupted()) { // interrupt() 메서드 호출됨
