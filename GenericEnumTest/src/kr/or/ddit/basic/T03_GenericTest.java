...
public class T03_GenericTest {
	 ...
	 * 제너릭 클래스 클래스를 만드는 방법
	 * 
	 * 형식) class 클래스명<제너릭타입 글자> { 제너릭 타입글자 변수명; // 변수선언에 제너릭을 사용하는 경우 ...
	 * 
	 * 제너릭타입글자 메서드명() { // 반환값이 있는 메서드에서 사용 ... return 값; } ... } -- 제너릭타입글자 -- T =>
	 * Type K => Key V => Value E => Element(자료 구조에 들어가는 항목들을 나타낼 때 사용)
	 */
