...
public class T03_JSONParsingExam {
	...
	public void parse() {
		...
		try {
			...
			url = new URL("http://211.237.50.150:7080/openapi/" + apiKey + "/json/" + svcKey + "/" + startIdx + "/"
					+ endIdx + "?RECIPE_ID=" + recipeId);
			...
			if (code.equals("INFO-000")) { // 정상이면
				...
				for (Object obj : list) {
					...
					String formatStr = String.format("%3s %8s %10s %10s %8s", jObj.get("ROW_NUM"), recipeId, jObj.get("IRDNT_NM"), jObj.get("IRDNT_CPCTY"), jObj.get("IRDNT_TY_NM"));
