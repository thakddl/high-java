...
public class T07_Equals_HashCodeTest {
	 ...
	 * 해시 함수에 의해 얻어지는 값은 해시값, 해시코드, 해시 체크섬 또는 간단하게 해시라고 한다.
	 * 
	 * HashSet, HashMap, HashTable과 같은 객체들을 사용할 경우...
	 * 객체가 서로 같은지를 비교하기 위해 equals()와 hashCode()메서드를 호출한다.
	 * 그래서 객체가 서로 같은지 여부를 결정하려면 두 매서드를 재정의 해야한다.
	 * HashSet, HashMap, HashTable에서는 객체가 같은지 여부는 데이터를 추가할 때 검사한다.
	 * 
	 * - equals()매서드는 두 객체의 내용(값)이 같은지 비교하는 매서드이고, 
	 *   hashCode()매서드는 객체에 대한 해시코드 값을 반환하는 매서드이다.
	 *   
	 * - equals()매서드와 hashCode()매서드에 관련된 규칙
	 *	 1. 두 객체가 같으면 반드시 같은 해시코드를 가져야 한다.
	 * 	 2. 두 객체가 같으면 equals매서드를 호출했을때 true를 반환해야 한다.
	 *    	즉, 객체 a, b가 같으면 a.equals(b)와 b.equals(a) 둘 다 true어야 한다. 
	 *	 3. 두 객체의 해시코드가 같다고 해서 두 객체가 반드시 같은 객체는 아니다.
	 *  	하지만, 두 객체가 같다면 반드시 해시코드가 같아야 한다.
	 *	 4. equals()매서드를 override하면 반드시 hashCode()매서드도 override해야한다.
	 *	 5. hashCode()는 기본적으로 Heap메모리에 있는 각 객체에 대한 메모리 주소를 매핑정보를 기반으로 한 정수값을 반환한다. 
